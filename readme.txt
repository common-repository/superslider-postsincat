...
Tags: widget, posts, category, animation, animated, gallery, scroller, mootools 1.4, mootools, slider, superslider
Requires at least: 3
Tested up to: 3.3.1
Stable tag: 2.0

This widget dynamically creates a list of posts from the active category. Displaying the first image and title in a scrolling list. 
...
If you have any problems or suggestions regarding this plugin [please speak up](http://support.superslider.daivmowbray.com/forum "support forum")
...
* [Demo 1](http://superslider.daivmowbray.com/wp-plugin-demos/superslider-postsincat/postsincat-elk "Demo")
...
* Add: is_single() || is_category() or just one of those: is_single() to the Widget logic field. (when using widget logic plugin)
...
2.0 has some changes to widget data.
...
* Please use the support system at http://wordpress.org/
...
* 2.0 (2012/11/22)
    ...
    * updated to run on mootools 1.4.5
    * updated the widget logic to WP 3+
    * cleaned the code

* 1.7 (2011/12/15)

    * updated to run on mootools 1.4
    * adjustments to the css files
    * changes to the file storage options
    * added number of random images option for posts with no image
    * fixed image size to use option
